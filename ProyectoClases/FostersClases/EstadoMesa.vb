...
#Region "Constantes"
  Private Const _INSERT As String = "GestionEstadosMesa.Insertar"
  Private Const _DELETE As String = "GestionEstadosMesa.Borrar"
  Private Const _UPDATE As String = "GestionEstadosMesa.Modificar"
#End Region
...
#Region "Funciones"
  ''' <summary>
  ''' Carga un estado de mesa dado un código
  ''' </summary>
  ''' <param name="codigo">Código del estado a buscar</param>
  ''' <returns>El estado de mesa encontrado</returns>
  ''' <author>Andrés Marotta, David Martínez Pérez</author>
  Public Shared Function Cargar(ByVal codigo As Integer) As EstadoMesa
    Dim Estado As New EstadoMesa
    Dim OrigenDatos As New BBDD
    Dim Lector As OracleDataReader

    If OrigenDatos.Conectar Then
      Lector = OrigenDatos.Consultar("SELECT * FROM Estados_Mesa WHERE codigo = " & codigo)

      If Lector IsNot Nothing AndAlso Lector.HasRows Then
        Lector.Read()
        Estado._Codigo = CInt(Lector(0))
        Estado._Nombre = CStr(Lector(1))
        Lector.Close()
      Else
        Estado = Nothing
      End If
  ...
  ''' <summary>
  ''' Carga todos los estados de mesa almacenados
  ''' </summary>
  ''' <returns>Un DataSet con todos los estados de mesa</returns>
  ''' <author>Andrés Marotta</author>
  Public Shared Function CargarDatos() As DataSet
    Dim Estados As New DataSet

    Estados = BBDD.CargarDatos("Estados_Mesa")

    Return Estados
  End Function

  ''' <summary>
  ''' Actualiza los datos de la tabla Estados_Mesa con los cambios de un DataSet dado
  ''' </summary>
  ...
  Public Shared Function Actualizar(ByVal estados As DataSet) As Boolean
    ...
    Try
      If Adaptador.Update(estados, "Estados_Mesa") >= 0 Then
        Ok = True
      Else
        Ok = False
      End If
